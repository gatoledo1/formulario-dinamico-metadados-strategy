...
export const dadosPessoais = (formik) => {
  return [
    {
      type: "section",
    ...
    },
    {
      ...
      value: formik.values?.dataNascimento
        ? moment(formik.values?.dataNascimento, "DD/MM/YYYY").toDate()
        : undefined,
      inputProps: {
      ...
      onChange: (value) => {
        ...
        formik.setFieldValue("dataNascimento", value ? valorFormatado : "");
    ...
    },
    {
      ...
      error:
        formik.touched?.nacionalidade && Boolean(formik.errors?.nacionalidade),
    ...
    },
    {
      ...
      initialOptions: (term) =>
        get(`/profissao/autocomplete?term=${term || ""}&pagina=0`),
    },
    {
      type: "inputTextOrNumber",
      xs: 12,
    ...
    },
    {
      ...
      initialOptions: (term) => {
        ...
        return idUf
          ? get(
              `/cidade/findByUnidadeFederativaAutocomplete?term=${term || ""}&idUf=${idUf}&pagina=0`,
            )
          : null;
      },
      ...
      dependencyValueToUpdate:
        formik.values?.cidadeResidencia?.unidadeFederativa,
    },
    {
      type: "inputTextWithButtonInside",
      condition:
        formik.values.cidadeResidencia?.unidadeFederativa?.pais?.nome !== "" &&
        formik.values.cidadeResidencia?.unidadeFederativa?.pais?.nome?.includes(
          "BRASIL",
        ),
      xs: 12,
      sm: 4,
      ...
      iconActionBtn: async () => {
        try {
          const dataCep = await get(
            `https://viacep.com.br/ws/${formik.values?.cep}/json/`,
          );
          if (dataCep) {
            const { logradouro, complemento, bairro, localidade, uf } = dataCep;
            try {
              let cidadeResidencia = await get(
                `/cidade/findCidadeByNomeAndUf?cidade=${localidade}&uf=${uf}&pagina=0`,
              );
      ...
      onInput: (e) => (e.target.value = e.target.value.replace(/[^0-9]/g, "")),
      onChange: (e) =>
        formik.setFieldValue("cep", e?.target?.value.replace(/[-.]/g, "")),
      validate: (value) => {
        if (!value) return jQuery.i18n.prop("label.campo.obrigatorio");
        return "";
      },
